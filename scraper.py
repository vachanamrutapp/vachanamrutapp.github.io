#!/usr/bin/env python3
"""
Scraper for English Vachanamrut from anirdesh.com
Creates JSON files matching the Gujarati structure
"""
...
def scrape_vachanamrut(vach_num):
    ...
    # headings[4] = "Gadhada I-1" (vachanamrut)
    # headings[5] = "Continuously Engaging One's Mind on God" (title)
    # paragraphs[3] = Setting
    # paragraphs[4:] = Text (until "Vachanamrut")
    ...
    for p in paragraphs[4:]:
        # Stop if we encounter "Vachanamrut" at the start
...
def main():
    # Fetch all 262 Vachanamruts
    output_dir = "assets/data/english"
    
    print("Starting to scrape 262 Vachanamruts...")
    print("This will take approximately 13-15 minutes with delays.\n")
    
    for vach_num in range(1, 263):  # 1 to 262
        try:
            ...
            if data:
                ...
                print(f"✓ Saved: vachanamrut-{vach_num}.json ({vach_num}/262)")
            
            # Add delay to avoid getting blocked (2.5 seconds between requests)
            if vach_num < 262:
                time.sleep(2.5)
    ...
    print(f"\n✓ Completed! Scraped {vach_num} Vachanamruts.")
